...
// auditor would try:
//   - double-@ email "a@b@stellar_card.com"
//   - substring injection "a@stellar_card.comevil.com"
//   - subdomain "a@bar.stellar_card.com"
//   - trailing dot "a@stellar_card.com."
//   - case variations
...
describe('requireInternal — domain allow', () => {
  ...
  it('allows any @stellar_card.com email', () => {
    const mw = freshMiddleware();
...
describe('requireInternal — domain suffix bypass guards', () => {
  ...
  it('rejects substring injection: a@stellar_card.comevil.com', () => {
    const mw = freshMiddleware();
    const { statusCode } = runMiddleware(mw, { id: 'u1', email: 'a@stellar_card.comevil.com' });
    assert.equal(statusCode, 403);
  });

  it('rejects subdomain: a@bar.stellar_card.com', () => {
    // endsWith('@stellar_card.com') on 'a@bar.stellar_card.com' — last 14
    // chars are 'r.stellar_card.com', doesn't match @stellar_card.com.
    const mw = freshMiddleware();
    const { statusCode } = runMiddleware(mw, { id: 'u1', email: 'a@bar.stellar_card.com' });
    assert.equal(statusCode, 403);
  });

  it('rejects trailing dot: a@stellar_card.com.', () => {
    const mw = freshMiddleware();
    const { statusCode } = runMiddleware(mw, { id: 'u1', email: 'a@stellar_card.com.' });
    assert.equal(statusCode, 403);
  });

  it('rejects lookalike tld: a@stellar_card.co', () => {
    const mw = freshMiddleware();
    const { statusCode } = runMiddleware(mw, { id: 'u1', email: 'a@stellar_card.co' });
    assert.equal(statusCode, 403);
  });

  it('rejects prefix injection: xstellar_card.com as suffix', () => {
    const mw = freshMiddleware();
    const { statusCode } = runMiddleware(mw, { id: 'u1', email: 'a@xstellar_card.com' });
    assert.equal(statusCode, 403);
  });
});
