...
process.env.CARDS402_CARD_REVEAL_EMAILS = 'test@stellar_card.com';
...
describe('GET /internal/orders/:id/card — F1 audit row shape', () => {
  ...
  it('writes the audit row with actor_role = operator role (F1), NOT a hard-coded string', async () => {
    ...
    assert.equal(auditRow.actor_email, 'test@stellar_card.com');
  ...
  it('different operator roles produce different actor_role rows (not a constant)', async () => {
    ...
    assert.equal(byEmail['owner-test@stellar_card.com'], 'owner');
    assert.equal(byEmail['admin-test@stellar_card.com'], 'admin');
  });
