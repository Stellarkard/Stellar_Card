...
// Boot order matters: start background loops before opening the HTTP
// listener so readiness checks don't go green while workers are still cold.
startJobs();
...
const server = app.listen(PORT, () => {
  console.log(
    `[stellar_card] backend running on port ${PORT} (${process.env.NODE_ENV || 'development'})`,
  );
});
...
//      stellar_card-api is configured with a longer kill_timeout in
//      the ecosystem file so 15 s gives comfortable headroom).
let shuttingDown = false;
function gracefulShutdown(signal) {
  if (shuttingDown) return;
  shuttingDown = true;
  console.log(`[stellar_card] ${signal} — draining and shutting down`);
  try {
    stopWatcher?.();
  } catch (err) {
    console.error('[stellar_card] stopWatcher error:', err);
  }
  try {
    stopJobs();
  } catch (err) {
  ...
  setTimeout(() => {
    console.error('[stellar_card] shutdown timeout — forcing exit');
    process.exit(1);
  }, 15_000).unref();
}
...
process.on('unhandledRejection', (reason) => {
  ...
  console.error(
    `[stellar_card] unhandledRejection: ${payload.name}: ${payload.message}${
      payload.stack ? `\n${payload.stack}` : ''
    }`,
  );
  try {
