...
//     time in middleware/requireInternal. A typo like `ops@stellar_cardcom`
...
const EnvSchema = z
  .object({
    ...
    // Database
    DB_PATH: z.string().default('./stellar_card.db'),
    ...
    // Public URL of this stellar_card instance — VCC uses it to construct the callback URL
    ...
    // Emails ending in @stellar_card.com are always allowed regardless of this setting.
  ...
  .superRefine((val, ctx) => {
    ...
    try {
      ...
      // `https://api.stellar_card.test` URL was mis-classified as a
