// Agent-local config file. Persisted at ~/.stellar_card/config.json after
// a successful `stellar_card onboard --claim` so the SDK can load the api
...
function defaultConfigDir(): string {
  return process.env.CARDS402_CONFIG_DIR || path.join(os.homedir(), '.stellar_card');
}
...
// mints keys as `stellar_card_<48 hex>`, which is pure ASCII alnum + underscore).
const API_KEY_SHAPE = /^[\x20-\x7e]+$/;

export function loadStellar_CardConfig(configPath?: string): Stellar_CardConfig | null {
  ...
  try {
    ...
    if (stat.isSymbolicLink()) {
      throw new Error(
        `stellar_card config at ${p} is a symbolic link. Refusing to load. ` +
          `Remove the link and re-run 'stellar_card onboard --claim <code>' to create a real file.`,
      );
    }
    if (!stat.isFile()) {
      throw new Error(
        `stellar_card config at ${p} is not a regular file. ` +
          `Remove it and re-run 'stellar_card onboard --claim <code>'.`,
      );
    }
    ...
    if (stat.size > MAX_CONFIG_BYTES) {
      throw new Error(
        `stellar_card config at ${p} is ${stat.size} bytes (max ${MAX_CONFIG_BYTES}). ` +
          `Refusing to load — the file is either corrupted or has been tampered with. ` +
          `Rotate your api key via the dashboard and re-run 'stellar_card onboard'.`,
      );
    }
    ...
    if (process.platform !== 'win32' && (stat.mode & 0o077) !== 0) {
      try {
        ...
        process.stderr.write(
          `⚠ stellar_card config at ${p} had loose permissions (${(stat.mode & 0o777).toString(8)}) — tightened to 600.\n` +
    ...
    const config = JSON.parse(raw) as Stellar_CardConfig;
    ...
    if (typeof config.api_key === 'string' && !API_KEY_SHAPE.test(config.api_key)) {
      throw new Error(
        `stellar_card config at ${p} contains an api_key with non-printable characters. ` +
          `The file may be corrupted or tampered with. Rotate your key and re-run 'stellar_card onboard'.`,
      );
    }
...
export function assertSafeBaseUrl(url: string, opts: { context?: string } = {}): string {
  ...
  // F4-config: reject embedded userinfo. `https://api.stellar_card.com/v1@evil.com/`
  // parses as username='api.stellar_card.com/v1', password='', hostname='evil.com'
  // — the whole string looks plausibly stellar_card-ish in log output, but every
  // request would go to evil.com carrying the user's api_key in the
  // Authorization header. There's no legitimate reason for a stellar_card base
 ...
 * truncated new one that loadStellar_CardConfig would explode on.
 *
...
export function saveStellar_CardConfig(
  ...
  // it actually CREATES. An existing ~/.stellar_card directory at 0755
  ...
  try {
    ...
    // rename commits. A leaked ~/.stellar_card/config.json.tmp-* file
...
export function resolveCredentials(
  ...
  if (!apiKey || !baseUrl) {
    ...
    const cfg = loadStellar_CardConfig();
    if (cfg) {
