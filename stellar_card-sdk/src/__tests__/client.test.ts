...
describe('Stellar_CardClient.createOrder', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it('sends correct headers and body', async () => {
    ...
    expect((opts.headers as Record<string, string>)['X-Api-Key']).toBe('stellar_card_test_key');
  ...
  it('strips trailing slash from baseUrl', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValue({ ok: true, status: 201, json: async () => ORDER_RESPONSE });
    global.fetch = fetchMock;
    const c = new Stellar_CardClient({ baseUrl: 'http://localhost:3000/v1/', apiKey: 'k' });
...
describe('Stellar_CardClient.getOrder', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  });
...
describe('Stellar_CardClient.waitForCard', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
    vi.useFakeTimers();
  });
...
describe('Stellar_CardClient.listOrders', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  });
...
describe('Stellar_CardClient.getUsage', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  });
