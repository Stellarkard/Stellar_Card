    ...
    <PageContainer>
      ...
      <Card title="Integration snippets">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
          ...
          >
            The <code>stellar_card</code> npm package ships a CLI for onboarding and a TypeScript SDK
            for purchases. See{' '}
            <a
              href="https://stellar_card.com/docs"
              target="_blank"
              rel="noreferrer"
          ...
          <Snippet title="Install" code={`npm install stellar_card@latest`} />
          <Snippet
            title="Onboard an agent"
            code={`npx -y stellar_card@latest onboard --claim <claim-code>`}
          />
          <Snippet
            title="Purchase a card"
            code={`import { purchaseCardOWS } from 'stellar_card';
...
function verify(req, secret) {
  const ts = req.headers['x-stellar_card-timestamp'];
  const sig = req.headers['x-stellar_card-signature']?.replace('sha256=', '');
