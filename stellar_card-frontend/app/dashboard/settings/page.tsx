...
const NOTIF_PREF_KEY = 'stellar_card.notifications';
const DENSITY_KEY = 'stellar_card.density';
...
export default function SettingsPage() {
  ...
  function exportData() {
    ...
    const data = {
      ...
      agentGroups: window.localStorage.getItem('stellar_card.agent_groups'),
    };
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `stellar_card-settings-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  ...
  function clearLocalData() {
    ...
    window.localStorage.removeItem('stellar_card.agent_groups');
    window.localStorage.removeItem(NOTIF_PREF_KEY);
    window.localStorage.removeItem(DENSITY_KEY);
    window.localStorage.removeItem('stellar_card.theme');
...
// Platform-owner-only treasury card. Renders the stellar_card Stellar treasury
